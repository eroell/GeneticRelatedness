...
class bigKNN(BaseEstimator):
    ...
    def fit(self,train_dataset,test_dataset,X_phenotype=None,y_phenotype=None): #fit(self,X_file,y_file=None,X_phenotype=None,y_phenotype=None):
        ...
        if self.metric == 'precomputed':
            ...
            for y_batch in tqdm(test_dataloader,desc='Y progress',leave=True):
                ...
                if count == 2:
                    print(f"time for loading first batch: {t.time()-beforeloop}")
    ...
    def _own_cosine(self,Xtrain,Xval,p):
        ...
        if torch.cuda.is_available() and self.device == 'gpu':
            ...
            res += 1
            print(f"GPU Cosine: {t.time()-start}")
        else:
            ...
            res *= -1 # 1-res without copy
            res += 1
            print(f"CPU Cosine: {t.time()-start2}")
        return res
    ...
    def _neighbors(self,SNP,dataloader,X_phenotype):
        ...
        for x_batch in tqdm(dataloader,desc='X for y progress',leave=False):
            ...
            print(f"time to put batch to variables: {t.time() - tovar}")
            # if torch.cuda.is_available():
            #     x_SNP = x_SNP.cuda()
            # else:
            tonp = t.time()
            ...
            x_Sex = x_Sex.numpy()
            x_Age = x_Age.numpy()
            print(f"time to put hdf5 format to np array: {t.time() - tonp}")
            ...
            if torch.cuda.is_available() and self.device == 'gpu':
                ...
                print(f"time to put tensor to cuda: {t.time() - tocuda}")
                ...
                print(f"time for entire function call: {t.time() - funcall}")
                
                tocpu = t.time()
                line_SNP[:,start_idx:start_idx+x_SNP.shape[0]] = distmatrix.cpu().numpy()
                print(f"time to put tensor to cpu: {t.time() - tocpu}")
        ...
        if self.n_neighbors == self.x_len:
            ...
            print(f"Type of line_SNP: {type(line_SNP)}")
            print(f"Type of element of line_SNP: {type(line_SNP[0,0])}")
            print(f"line_SNP.shape: {line_SNP.shape}")
            print(f"line_SNP.itemsize): {line_SNP.itemsize}")
            sorting = t.time()
            ...
            print(f"time for partitioning: {t.time()-sorting}")
