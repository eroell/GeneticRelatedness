...
##############################################################################
'''
A main script for comparing the bigKNN_PCA_enabled's performance to sklearns KNeighborsRegressor
'''
...
from src.KNN_PCA_enabled_II import bigKNN_PCA_enabled
...
if __name__ == '__main__':
    ...
    elif args.w_state is None or args.w_state == 'None':
        if args.dataset == "small_toy" or args.dataset == "toy":
            w = np.ones(100)
        elif args.dataset == "10k_bellot":
            w = np.ones(10000)
    ...
    if args.metric == "mahalanobis":
        ...
        with h5.File(os.path.join(args.intermediate_dir, pca_fyle), 'r') as hf:
        #    # hf = h5.File(os.path.join(intermediates_path, "cov_mat_10k_bellot_device_cpu_batchsize_400.hdf5"), 'r')
        #    # cov_loaded = np.array(hf['/covariance_matrix'])
        ...
        logging.info("mahalanobis loaded inverse covariance matrix fit to model")
