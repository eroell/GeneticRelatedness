...
if __name__ == '__main__':
    ...
    parser.add_argument(
        ...
        default=pathlib.Path(__file__).resolve().parents[1] / 'intermediates',
                                                               #/ name,
        type=str,
        help='intermediate_dir path for storing the results.'
    )
    ...
    intermediate_dir_filename = f'pca_intermediates_{args.dataset}_device_{args.device}.hdf5'
    
    force_write = True
    if not os.path.exists(os.path.join(args.intermediate_dir, intermediate_dir_filename)) or force_write:
        ...
        with h5.File(os.path.join(args.intermediate_dir, intermediate_dir_filename), 'w') as f:
            f.create_dataset("means",data=means_bigPCA)
