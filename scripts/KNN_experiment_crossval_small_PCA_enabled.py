...
def cross_val_score_pca(model,dataset,n_splits=5,random_state=47,
    ...
    for i,split in enumerate(split_lys):
        ...
        pca_fyle = 'pca_intermediates_cvsplit_'+str(i)+'_dataset_small_random_state_47_device_cpu.hdf5'
...
if __name__ == '__main__':
    ...
    filename = f'evaluate_metrics_{args.dataset}_n_components_{args.n_components}_n_neighbours_{args.n_neighbors}_weights_{args.weights}_metric_{args.metric}_p_{args.p}_adjusted_{args.unadjusted}_njobs_{args.n_jobs}_batchsize_{args.batch_size}_device_{args.device}.csv'
