...
This script tests these two implementations on synthetic dataset 2.
Consult the dataset creation scripts in this folder for details on synthetic dataset 2.
...
prior to KNN regression leads to improved KNN regression accuracy (reduced MAE)
on a synthetic dataset with few meaningful and many noisy features.
'''
...
if __name__ == '__main__':
    ...
    tot_components = 20
    MAEs = []
    for n_components in range(tot_components+1):
        ...
        MAEs.append(model.evaluate_metrics()['MAE'])
        if n_components in [20,15,10,5,2,1,0]:
            print('\n----------------------------\n',
                  f'KNN Prediction Performance on dataset {args.dataset} using {n_components} principal component(s):\n',
                  model.evaluate_metrics(),
                  '\n----------------------------\n')
    
    # Plot the the Mean Absolute Error on KNN Regression on synthetic data, which is minimal when using 1 or 2 PC's
    fig, ax = plt.subplots()
    ax.plot(range(tot_components+1),MAEs)
    ax.set_title("Mean Absolute Error of KNN Regression predictions\n with prior PCA on data")
    ax.set_ylabel('MAE')
    ax.set_xlabel('PCs used')
    ax.set_xlim(21,-1)
    ax.set_xticks(range(tot_components+1))
